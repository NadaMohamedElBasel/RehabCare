...
function Prescriptions() {
  ...
  const fetchPrescriptions = async () => {
    try {
      ...
      const res = await axios.get(
        `http://127.0.0.1:5000/api/prescriptions/${patientId}`
      );
  ...
  const updateStatus = async (prescriptionId, newStatus) => {
    try {
      await axios.put(
        `http://127.0.0.1:5000/api/prescriptions/${prescriptionId}`,
        { status: newStatus }
      );
      fetchPrescriptions();
    } catch (err) {
            ...
            <div key={med.prescription_id} className="prescription-card">
              ...
              <div className="progress-controls">
                <select
                  ...
                  onChange={(e) =>
                    updateStatus(med.prescription_id, e.target.value)
                  }
                >
          ...
          {exercises.map((exercise) => (
            ...
            >
              ...
              <div className="card-body">
                ...
                {exercise.dosage && (
                  <p><strong>Sets/Reps:</strong> {exercise.dosage}</p>
                )}
              ...
              <div className="progress-controls">
                <select
                  ...
                  onChange={(e) =>
                    updateStatus(exercise.prescription_id, e.target.value)
                  }
                >
